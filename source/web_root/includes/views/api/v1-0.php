<?php
	...
	// retrieve input parameters
		...
		$allowableFilters = array('public_id', 'keywords', 'country', 'status', 'tag_id', 'page');
	...
	// check usage cap
		if (!count($errors) && !$user[0]['unlimited_api_queries']) {
			if ($user[0]['internal_api_calls_today'] >= $apiCap) $errors[count($errors)] = 4; // maximum query limit achieved
			elseif ($user[0]['internal_api_calls_today'] >= ($apiCap - intval($apiCap / 20))) $warnings[count($warnings)] = 1; // approaching maximum queries
		}
	...
	// display data
		...
		for ($counter = 0; $counter < count($warnings); $counter++) {
			...
			$xmlOutput .= "    <human_readable_warning>" . $warningCodes[$warnings[$counter]] . "</human_readable_warning>\n";
		}
		$xmlOutput .= "  </warnings>\n";
		$xmlOutput .= "  <errors>\n";
		for ($counter = 0; $counter < count($errors); $counter++) {
			$xmlOutput .= "    <error_code>" . $errors[$counter] . "</error_code>\n";
			$xmlOutput .= "    <human_readable_error>" . $errorCodes[$errors[$counter]] . "</human_readable_error>\n";
		}
		$xmlOutput .= "  </errors>\n";
		if (!$error) {
			...
			for ($counter = 0; $counter < count($data); $counter++) {
				...
				$xmlOutput .= "      <status><![CDATA[" . $rumourStatuses[$data[$counter]['status']] . "]]></status>\n";
