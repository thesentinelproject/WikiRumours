<?php
	...
	// filters
		...
		// export button
			...
			echo "      <a href='/export/" . urlencode($keyvalue_array->updateKeyValue($tl->page['parameter1'], 'report', 'users', '|')) . "' target='_blank'>Export</a>\n";
			echo "    </div>\n";
		echo "  </div>\n";
		echo $form->end() . "\n";
	...
	// pagination
		if ($numberOfPages > 1) {
			...
			echo $form->paginate($filters['page'], $numberOfPages, '/admin_users/' . $keyvalue_array->updateKeyValue($tl->page['parameter1'], 'page', '#', '|'));
			echo "    </div>\n";
			echo "  </div>\n";
		}
