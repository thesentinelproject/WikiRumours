<?php
	...
	/* Country */			echo $form->row('country', 'country_id', $operators->firstTrue(@$_POST['country_id'], @$sighting[0]['sighting_country_id'], (@!$id && @$pseudonym['country_id'] ? $pseudonym['country_id'] : false)), true, 'Country where heard', 'form-control');
	/* Community */			echo $form->row('text', 'city', $operators->firstTrue(@$_POST['city'], @$sighting[0]['sighting_city']), false, 'Community', 'form-control', null, 50);
							echo $form->row('latlongmap', 'heard_at', $operators->firstTrue((floatval(@$_POST['heard_at_latitude']) <> 0 || floatval(@$_POST['heard_at_longitude']) <> 0 ? floatval(@$_POST['heard_at_latitude']) . "," . floatval(@$_POST['heard_at_longitude']) : false), (floatval(@$sighting[0]['sighting_latitude']) <> 0 && floatval(@$sighting[0]['sighting_longitude']) <> 0 ? @$sighting[0]['sighting_latitude'] . "," . @$sighting[0]['sighting_longitude'] : false), (@!$id && @$pseudonym['latitude'] && @$pseudonym['longitude'] ? $pseudonym['latitude'] . ',' . @$pseudonym['longitude'] : false)));
