	...
	function retrieveTags($matching = null, $containing = null, $otherCriteria = null, $sortBy = false, $limit = false) {
		...
		// build query
			if ($matching['rumour_id']) {
				...
				$query .= " (SELECT COUNT(rumour_id) FROM " . $tablePrefix . "rumours_x_tags WHERE " . $tablePrefix . "rumour_x_tags.tag_id = " . $tablePrefix . "tags.tag_id) as number_of_rumours";
			...
			$result = $dbConnection->query($query) or die('Unable to execute retrieveTags: ' . $dbConnection->error . '<br /><br />' . $query);
