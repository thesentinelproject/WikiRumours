			...
			if ($_POST['formName'] == 'addEditContentForm' && @$this->cms_id && @$this->content_type && $_POST['deleteContent'] == 'Y') {
				...
				// delete file, if required
					if ($this->content_type == 'f' || $this->content_type == 'd') {
						// delete file
							$success = unlink ($this->cms[0]['destination_url']);
							if (!$success || file_exists($this->cms[0]['destination_url'])) $tl->page['error'] .= "Unable to delete associated file for some reason. ";
							else {
								// delete folder
									rmdir (substr($this->cms[0]['destination_url'], 0, strrpos($this->cms[0]['destination_url'], '/')));
							}
					}
